...
The main reason for the existence of this tool is that the author
dislikes filling web forms on a regular basis.

Most arguments are parsed as author names. Necessary spaces in author
names may be given as underscores `_` to avoid having to type quotes
around the name. To be more specific than just a last name, a first
name or initial can be given like first.last (separated by dot) or
last,first (separated by comma). Only the initial letter of the first
name is significant, so last,f and last,first are equivalent.
